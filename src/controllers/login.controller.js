...
class LoginController {
  ...
  async register(req, res) {
    try {
      ...
      const newUser = await userService.createUser(body); // Crear un nuevo usuario en la base de datos
      res.status(201).json(newUser); // Devolver una respuesta con el nuevo usuario creado
    } catch (error) {
      res.status(500).json(error.message);
    }
  }
